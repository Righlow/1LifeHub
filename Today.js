...
import {
  ...
  Animated, Dimensions, RefreshControl, FlatList,
} from 'react-native';
...
function GlassCard({ children, style, accentColor, onPress }) {
  ...
  return (
    <Wrapper onPress={onPress} activeOpacity={0.85} style={{ marginHorizontal: 14, marginBottom: 10 }}>
      <BlurView intensity={40} tint="dark" style={[s.glassCard, style]}>
        <View style={s.glassShine} pointerEvents="none" />
        {accentColor && <View style={[s.accentLine, { backgroundColor: accentColor }]} />}
        {children}
      </BlurView>
    </Wrapper>
  );
...
function HabitRow({ habit, completed, onToggle }) {
  ...
  return (
    <Animated.View style={[s.habitRow, { transform: [{ scale }] }]}>
      <TouchableOpacity onPress={handleToggle} style={s.habitTouchable} activeOpacity={0.7}>
        <View style={[s.checkbox, completed && { backgroundColor: color, borderColor: color }]}>
          {completed && <Text style={s.checkTick}>✓</Text>}
        </View>
        <View style={{ flex: 1 }}>
          <Text style={[s.habitName, completed && s.habitDone]}>{habit.name}</Text>
          <Text style={[s.habitXP, { color }]}>+{habit.xp_value || 10} XP</Text>
        </View>
...
function HealthSnapshot({ health, onPress }) {
  const metrics = [
    { label: 'Sleep', value: health?.sleep    || 0, unit: 'hrs',  color: BLUE,  max: 8  },
    { label: 'Water', value: health?.water    || 0, unit: 'gl',   color: GREEN, max: 8  },
    { label: 'Move',  value: health?.movement || 0, unit: 'min',  color: AMBER, max: 60 },
  ];
        ...
        {metrics.map(m => (
          <View key={m.label} style={s.healthMetric}>
            ...
            <View style={s.healthTrack}>
              <View style={[s.healthFill, { width: `${Math.min((m.value / m.max) * 100, 100)}%`, backgroundColor: m.color }]} />
            </View>
          </View>
        ))}
      </View>
...
function WeeklyGoalBar({ goals, onPress }) {
  const weekly = goals.filter(g => g.timeframe === 'weekly' && (g.status === 'active' || !g.status));
  const avg    = weekly.length
    ? Math.round(weekly.reduce((s, g) => s + (g.progress || 0), 0) / weekly.length)
    : 0;
  ...
  return (
    <GlassCard onPress={onPress} style={s.goalCard}>
      ...
      <Text style={s.goalSub}>{weekly.length} active weekly goal{weekly.length !== 1 ? 's' : ''} · Tap to manage</Text>
    </GlassCard>
  );
}
...
export default function TodayScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    const activeHabits = (Array.isArray(h) ? h : []).filter(x => x.is_active !== false);
    ...
    const todayEntry = (Array.isArray(e) ? e : []).find(x => x.date === todayStr);
    if (todayEntry?.habit_completions) {
      const comp = {};
      todayEntry.habit_completions.forEach(c => { comp[c.habit_id] = c.completed; });
      setCompletions(comp);
    }
  }, [todayStr]);
  ...
  const toggleHabit = async (habitId) => {
    ...
    const domain_xp = {};
    habit_completions.filter(c => c.completed && c.domain).forEach(c => {
      domain_xp[c.domain] = (domain_xp[c.domain] || 0) + c.xp_value;
    });

    const data = { date: todayStr, habit_completions, domain_xp, total_xp_earned };
  ...
  const habitScore      = activeHabits.length ? completedCount / activeHabits.length : 0;
  const healthScore     = health
    ? (Math.min((health.sleep || 0) / 8, 1) + Math.min((health.water || 0) / 8, 1) + Math.min((health.movement || 0) / 60, 1)) / 3
    : 0;
  const weeklyGoals     = goals.filter(g => g.timeframe === 'weekly' && (g.status === 'active' || !g.status));
  const goalScore       = weeklyGoals.length
    ? (weeklyGoals.reduce((s, g) => s + (g.progress || 0), 0) / weeklyGoals.length) / 100
    : 0;
  ...
  const bloomedDomains = [...new Set(
    activeHabits.filter(h => completions[h.id] && h.domain).map(h => h.domain)
  )];
  ...
  return (
    ...
    >
      ...
      <View style={s.sectionHeader}>
        ...
        <Text style={s.secCount}>{completedCount} / {activeHabits.length}</Text>
      </View>

      {activeHabits.length === 0 ? (
        <GlassCard onPress={() => navigation.navigate('Habits')} style={s.emptyCard}>
...
const s = StyleSheet.create({
  ...
  bgGlow:        { position: 'absolute', width: 300, height: 300, backgroundColor: COLORS.neonSoft, borderRadius: 150, top: -120, right: -80 },
  ...
  title:         { fontSize: 20, fontFamily: 'Orbitron', color: GREEN, letterSpacing: 1 },
  sub:           { fontSize: 10, color: COLORS.textMuted, marginTop: 2 },

  streakPill:    { flexDirection: 'row', alignItems: 'baseline', backgroundColor: 'rgba(0,255,135,0.08)', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: 'rgba(0,255,135,0.18)' },
  streakNum:     { fontSize: 18, fontWeight: '900', color: GREEN },
  streakLbl:     { fontSize: 10, color: GREEN, opacity: 0.7 },

  glassCard:     { borderRadius: 20, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.card, padding: 16 },
  glassShine:    { position: 'absolute', top: 0, left: 0, right: 0, height: '40%', backgroundColor: 'rgba(255,255,255,0.03)', borderTopLeftRadius: 20, borderTopRightRadius: 20 },
  accentLine:    { position: 'absolute', top: 0, left: 0, right: 0, height: 2 },

  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 18, paddingTop: 6, paddingBottom: 6 },
  secLabel:      { paddingHorizontal: 18, paddingTop: 6, paddingBottom: 6, fontSize: 9, color: COLORS.textMuted, letterSpacing: 2, fontWeight: '700' },
  ...
  habitTouchable:{ flexDirection: 'row', alignItems: 'center', padding: 10, gap: 12 },
  habitRow:      { borderRadius: 12 },
  checkbox:      { width: 26, height: 26, borderRadius: 8, borderWidth: 2, borderColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
  ...
  divider:       { height: 1, backgroundColor: 'rgba(255,255,255,0.04)', marginHorizontal: 10 },

  healthCard:    { padding: 14 },
  cardTitle:     { fontSize: 11, color: COLORS.textMuted, fontWeight: '700', letterSpacing: 1, marginBottom: 12 },
  ...
  healthLabel:   { fontSize: 9, color: COLORS.textMuted, letterSpacing: 0.5, marginBottom: 6 },
  healthTrack:   { width: 40, height: 3, backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 2, overflow: 'hidden' },
  healthFill:    { height: '100%', borderRadius: 2 },
  healthTap:     { fontSize: 9, color: COLORS.textMuted, textAlign: 'right', marginTop: 8 },

  goalCard:      { padding: 14 },
  goalCardRow:   { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  goalAvgPct:    { fontSize: 20, fontWeight: '900' },
  goalTrack:     { height: 6, backgroundColor: 'rgba(0,0,0,0.4)', borderRadius: 4, overflow: 'hidden', marginBottom: 8 },
